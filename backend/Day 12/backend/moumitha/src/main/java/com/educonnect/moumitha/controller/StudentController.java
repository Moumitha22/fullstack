...
public class StudentController {
    ...
    @PutMapping(UPDATE_DETAILS + "/{email}")
    @PreAuthorize(STUDENT_UPDATE)
    public ResponseEntity<?> updateStudentByEmail(@RequestBody UpdateStudentRequest request, @PathVariable String email) {
        
        try {
            var response = studentService.updateDetails(request, email);
            return new ResponseEntity<>(response, HttpStatus.OK);
