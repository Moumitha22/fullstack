...
public class InstituteController {
    ...
    public ResponseEntity<?> updateInstituteByEmail(@RequestBody UpdateInstituteRequest request, @PathVariable String email) {
        try {
            var response = instituteService.updateDetails(email, request);
            return new ResponseEntity<>(response, HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(new MessageResponse(), HttpStatus.EXPECTATION_FAILED);
        }
